...
static void validate_unique_magnetodynamics_model(
    ParticleMagnetodynamicsParameters const &magnetodynamics) {
  if (magnetodynamics.enabled_count() > 1) {
...
ParticleHandle::ParticleHandle() {
  ...
  add_parameters({
      ...
      {"_magnetodynamics_tsw",
       [this](Variant const &value) {
         set_particle_property([&value](Particle &p) {
           auto magnetodynamics = p.magnetodynamics();
           auto const dict = get_value<VariantMap>(value);
           if (dict.contains("is_enabled")) {
             magnetodynamics.tsw.is_enabled =
           ...
           if (dict.contains("sw_dt_incr")) {
             magnetodynamics.tsw.dt_incr =
                 get_value<double>(dict.at("sw_dt_incr"));
           }
           validate_unique_magnetodynamics_model(magnetodynamics);
           p.magnetodynamics() = magnetodynamics;
         });
       },
       [this]() {
         auto const &p = get_particle_data(m_pid);
      ...
      {"_magnetodynamics_egg",
       [this](Variant const &value) {
         set_particle_property([&value](Particle &p) {
           auto magnetodynamics = p.magnetodynamics();
           auto const dict = get_value<VariantMap>(value);
           if (dict.contains("is_enabled")) {
             magnetodynamics.egg.is_enabled =
                 get_value<bool>(dict.at("is_enabled"));
           }
           if (dict.contains("gamma")) {
             ...
             if (gamma <= 0.) {
               throw std::domain_error(error_msg(
                   "magnetodynamics.egg", "gamma must be a float > 0"));
             }
           ...
           if (dict.contains("axis_quat_body")) {
             magnetodynamics.egg.axis_quat_body = get_quaternion_safe(
                 "axis_quat_body", dict.at("axis_quat_body"));
           }
           validate_unique_magnetodynamics_model(magnetodynamics);
           p.magnetodynamics() = magnetodynamics;
         });
      ...
      {"_magnetodynamics_ideal",
       [this](Variant const &value) {
         set_particle_property([&value](Particle &p) {
           auto magnetodynamics = p.magnetodynamics();
           auto const dict = get_value<VariantMap>(value);
           if (dict.contains("is_enabled")) {
             magnetodynamics.ideal.is_enabled =
                 get_value<bool>(dict.at("is_enabled"));
           }
           if (dict.contains("sat_mag")) {
             ...
             if (sat_mag <= 0.) {
               throw std::domain_error(error_msg(
                   "magnetodynamics.ideal", "sat_mag must be a float > 0"));
             }
